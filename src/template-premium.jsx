  ...
  return (
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, background: "rgba(255,255,255,0.95)", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(14,165,233,0.2)", padding: "0 36px", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <div style={{ display: "flex", gap: 6, alignItems: "center" }} className="desktop-nav-p">
        {SECTIONS.map(id => (
          <button key={id} onClick={() => scrollTo(id)} style={{ background: "transparent", border: "none", cursor: "pointer", padding: "8px 13px", fontFamily: FONT_INTER, fontSize: 11, fontWeight: active === id ? 600 : 400, color: active === id ? ACCENT : "rgba(0,0,0,0.5)", transition: "all .3s", textTransform: "uppercase", letterSpacing: 1.5 }}>{NAV_LABELS[id]}</button>
        ))}
      </div>
      ...
      {menuOpen && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, width: "100%", minHeight: "100vh", zIndex: 9999, backgroundColor: "#ffffff", background: "#ffffff", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 24 }}>
          ...
          {SECTIONS.map(id => (
            <button key={id} onClick={() => scrollTo(id)} style={{ background: "transparent", border: "none", cursor: "pointer", padding: "18px 32px", fontFamily: FONT_INTER, fontSize: 26, fontWeight: active === id ? 600 : 400, color: active === id ? ACCENT : "rgba(0,0,0,0.5)", width: "80%", textAlign: "center", letterSpacing: 1 }}>{NAV_LABELS[id]}</button>
          ))}
        </div>
      )}
      <style>{`
  ...
  return (
    <section id="home" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#ffffff", padding: "100px 36px 60px", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: "min(1100px, 92vw)", width: "100%", textAlign: "center", position: "relative", zIndex: 1 }}>
        ...
        <p style={{ fontFamily: FONT_INTER, fontSize: "clamp(14px, 2vw, 16px)", color: "rgba(0,0,0,0.5)", maxWidth: 480, margin: "0 auto 44px", lineHeight: 1.9, fontWeight: 400, letterSpacing: 0.3 }}>Creiamo siti web che trasformano visitatori in clienti. Design premium, performance reali, risultati misurabili.</p>
        <div className="cta-hero-wrap" style={{ display: "flex", gap: 20, justifyContent: "center", flexWrap: "wrap" }}>
          ...
          <button style={{ background: "transparent", color: "rgba(0,0,0,0.5)", border: "1px solid rgba(0,0,0,0.15)", padding: "16px 44px", fontFamily: FONT_INTER, fontSize: 12, fontWeight: 400, cursor: "pointer", textTransform: "uppercase", letterSpacing: 3, minWidth: 200 }}>Vedi Portfolio</button>
        </div>
    ...
    <section id="problema" style={{ padding: "110px 36px", background: "#ffffff" }}>
      <div style={{ maxWidth: "min(1200px, 92vw)", width: "100%", margin: "0 auto", padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h2 style={{ fontFamily: FONT_INTER, fontSize: "clamp(28px, 4.5vw, 46px)", fontWeight: 800, color: "#111", marginBottom: 16 }}>Il Tuo Sito Ti Sta Costando Clienti</h2>
    ...
    <section id="soluzione" style={{ padding: "110px 36px", background: "#fafafa" }}>
      <div style={{ maxWidth: "min(1200px, 92vw)", width: "100%", margin: "0 auto", padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h2 style={{ fontFamily: FONT_INTER, fontSize: "clamp(28px, 4.5vw, 46px)", fontWeight: 800, color: "#111", marginBottom: 16 }}>Il Nostro <span style={{ color: ACCENT, fontStyle: "italic" }}>Approccio</span></h2>
          <div style={{ width: 40, height: 1, background: ACCENT, margin: "0 auto" }} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 28 }}>
          {benefits.map((b, i) => (
            <div key={i} style={{ background: "#ffffff", border: `1px solid ${ACCENT_RGBA(0.25)}`, padding: "44px 32px", textAlign: "center", boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
    ...
    <section id="risultati" style={{ padding: "110px 36px", background: "#ffffff" }}>
      <div style={{ maxWidth: "min(1200px, 92vw)", width: "100%", margin: "0 auto", padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h2 style={{ fontFamily: FONT_INTER, fontSize: "clamp(28px, 4.5vw, 46px)", fontWeight: 800, color: "#111" }}>Cosa Ottengono i Nostri Clienti</h2>
          <div style={{ width: 40, height: 1, background: ACCENT, margin: "20px auto 0" }} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 28 }}>
          {testimonials.map((t, i) => (
            <div key={i} style={{ padding: "40px 28px", borderLeft: `2px solid ${ACCENT}` }}>
    ...
    <section id="servizi" style={{ padding: "110px 36px", background: "#fafafa" }}>
      <div style={{ maxWidth: "min(1200px, 92vw)", width: "100%", margin: "0 auto", padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h2 style={{ fontFamily: FONT_INTER, fontSize: "clamp(28px, 4.5vw, 46px)", fontWeight: 800, color: "#111" }}>Cosa Facciamo</h2>
          <div style={{ width: 40, height: 1, background: ACCENT, margin: "20px auto 0" }} />
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 24 }}>
          {services.map((s, i) => (
            <div key={i} style={{ border: `1px solid ${ACCENT_RGBA(0.2)}`, padding: "40px 28px", textAlign: "center", transition: "all .3s", background: "#ffffff", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
    ...
    <section id="processo" style={{ padding: "110px 36px", background: "#ffffff" }}>
      <div style={{ maxWidth: "min(1000px, 92vw)", width: "100%", margin: "0 auto", padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h2 style={{ fontFamily: FONT_INTER, fontSize: "clamp(28px, 4.5vw, 46px)", fontWeight: 800, color: "#111" }}>Come Lavoriamo</h2>
          <div style={{ width: 40, height: 1, background: ACCENT, margin: "20px auto 0" }} />
        </div>
        <div style={{ display: "flex", flexDirection: "column" }}>
          {steps.map((s, i) => (
            <div key={i} style={{ display: "flex", gap: 40, alignItems: "flex-start", padding: "40px 0", borderBottom: i < 2 ? "1px solid rgba(14,165,233,0.15)" : "none" }}>
  ...
  return (
    <section id="pricing" style={{ padding: "110px 36px", background: "#fafafa" }}>
      <div style={{ maxWidth: "min(640px, 92vw)", width: "100%", margin: "0 auto", padding: "0 24px", textAlign: "center" }}>
        ...
        <h2 style={{ fontFamily: FONT_INTER, fontSize: "clamp(28px, 4.5vw, 46px)", fontWeight: 800, color: "#111", marginBottom: 16 }}>Quanto Costa un Sito che Funziona</h2>
        <div style={{ width: 40, height: 1, background: ACCENT, margin: "0 auto 48px" }} />
        <div style={{ border: `1px solid ${ACCENT_RGBA(0.25)}`, padding: "52px 40px", background: "#ffffff", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
    ...
    <section id="faq" style={{ padding: "110px 36px", background: "#ffffff" }}>
      <div style={{ maxWidth: "min(900px, 92vw)", width: "100%", margin: "0 auto", padding: "0 24px" }}>
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          ...
          <h2 style={{ fontFamily: FONT_INTER, fontSize: "clamp(28px, 4.5vw, 46px)", fontWeight: 800, color: "#111" }}>Domande Frequenti</h2>
          <div style={{ width: 40, height: 1, background: ACCENT, margin: "20px auto 0" }} />
        </div>
        {faqs.map((f, i) => (
